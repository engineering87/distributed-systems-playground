...
const BENOR_CODE = `// Ben-Or: randomized binary consensus in an asynchronous system.
// Each round has two phases. In the first everybody reports its value and waits
// for N - F reports: a value held by more than half becomes a proposal, otherwise
// the process proposes NONE. In the second everybody reports that proposal and
// waits for N - F of them: more than F equal proposals decide, a single one is
// adopted, none at all means the process tosses a coin.
// The coin is why FLP does not apply: no schedule can keep the processes apart
// for ever, because eventually they all toss the same way.
interface Consensus
  request Propose(v)
  indication Decide(v)
end
...
algorithm BenOr
  ...
  upon event ⟨cons, Propose | x⟩ where v = nil do
    v := x
    proposed := x
    r := 1
    phase := ONE
    forall q in Π do
      trigger ⟨net, Send | q, [VOTE, r, v]⟩
    end
  end
  ...
  upon event ⟨net, Deliver | q, [VOTE, k, x]⟩ do
    p1[[k, q]] := x
    call tryPhaseOne()
  end

  upon event ⟨net, Deliver | q, [PROP, k, x]⟩ do
    p2[[k, q]] := x
    call tryPhaseTwo()
  end

  // first phase: a value held by more than half becomes this round's proposal
  function tryPhaseOne()
    if phase = ONE and #{q in Π where p1[[r, q]] ≠ nil} ≥ N - F then
      w := NONE
      forall x in {0, 1} do
        if 2 * #{q in Π where p1[[r, q]] = x} > N then
          w := x
        end
      end
      phase := TWO
      forall q in Π do
        trigger ⟨net, Send | q, [PROP, r, w]⟩
      end
      call tryPhaseTwo()
    end
  end

  // second phase: more than F equal proposals decide, one is adopted, none means a coin
  function tryPhaseTwo()
    if phase = TWO and #{q in Π where p2[[r, q]] ≠ nil} ≥ N - F then
      support := nil
      forall x in {0, 1} do
        if #{q in Π where p2[[r, q]] = x} ≥ 1 then
          support := x
        end
        if #{q in Π where p2[[r, q]] = x} > F and decided = nil then
          decided := x
      ...
      // a process that has decided runs one more round, to carry its value to
      // the others, and then stops: without that round the slow ones would wait
      if decided ≠ nil then
        extra := extra + 1
      end
      if extra > 1 then
...
// Everybody decides — with probability one, not with certainty: this says only
// that it happened within the simulated duration.
property Termination eventually
  #{p in correct where decided[p] = nil} = 0
end
`;
...
EXAMPLES.push({
  key: 'ben-or',
  title: 'Ben-Or: consensus with a coin',
  scenario: catalogScenario({
    n: 5, seed: 3, preset: 'async', code: BENOR_CODE, top: 'BenOr',
    inputs: '0ms 1 Propose | 1\n0ms 2 Propose | 1\n0ms 3 Propose | 1\n0ms 4 Propose | 0\n0ms 5 Propose | 0',
    stopAt: '2s'
  })
});
...
const META = {
  ...
  'ben-or': { category: 'Consensus', summary: 'When the processes cannot agree, they toss a coin: randomness buys the termination that FLP denies.' }
};
